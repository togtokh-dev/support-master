 ...
 */
export const levelInfo = async (curPoints: number) => {
  for (let index = 1; index <= 8; index++) {
    const nextLevelThreshold = 5000 * (Math.pow(2, index) - 1);
    if (curPoints < nextLevelThreshold || index === 8) {
      const info = level_data[Math.min(index - 1, 7)];
      return { ...info, user_exp: curPoints };
    }
  }
  const last = level_data[7];
  return { ...last, user_exp: curPoints };
};

// ---------- ТӨРӨЛ / ИНТЕРФЕЙС ----------
export type PricingInput = {
  amount: number; // суурь тоо/үнэ (ж: USD дүн)
  profit: number; // ашиг (хувь эсвэл тогтмол дүн)
  profit_type: "percentage" | "fixed"; // ашигийн төрөл
  bonus: number; // ерөнхий бонусын хувь (ж: 5 => 5%)
};

export type VoucherDiscount = {
  discount_value: number; // хувь эсвэл тогтмол дүн
  discount_type: "percentage" | "fixed";
};

export type RankInfo = {
  rank: number; // эрэмбэ (коэффициент)
  point: number;
  name: string;
  user_exp: number;
};
...
export type ConvertArgs = {
  el: PricingInput;
  voucher_discount?: VoucherDiscount; // заавал биш
  currencyAmount: number; // ханш (ж: USD->MNT)
  rank: RankInfo;
};
...
export type ConvertResult = {
  ...
  discount: {
    default: {
      // Ерөнхий бонус — ҮНДСЭН ДҮН-с шууд
      amount: number;
      percent: number;
    };
    voucher: {
      // Ваучер — ҮНДСЭН ДҮН дээр
      amount: number;
      type: "percentage" | "fixed" | null;
      value: number;
    };
    rank: {
      // Rank — ЗӨВХӨН АШГИЙН ДҮНГЭЭС
      amount: number;
      rank: RankInfo;
    };
  };
  ...
  // --- Эцсийн дүн ---
  final_payable: number; // төлөх эцсийн дүн
  margin_over_converted: number; // converted_amount-тай харьцуулсан зөрүү
};
...
export const convert = async ({
  ...
  currencyAmount,
  rank,
}: ConvertArgs): Promise<ConvertResult> => {
  // 0) Оролтыг цэвэрлэх
