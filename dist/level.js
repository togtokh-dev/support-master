...
const convert = async ({ el, voucher_discount, currencyAmount, rank, }) => {
    ...
    const rank_discount_amount = profit_amount > 0 ? Math.round((profit_amount / 10) * rank.rank) : 0;
    ...
    // 6) Эцсийн дүн
    const final_payable = Math.round(clampMin(amount - default_discount_amount - voucher_amount - rank_discount_amount));
